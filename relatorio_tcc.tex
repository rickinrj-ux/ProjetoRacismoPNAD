...
(i)~\textit{barreira de acesso}: OR~$=0,674$ para CBO~1--4
(GLMM, $N=7.694.198$);
(ii)~\textit{barreira salarial intra-ocupação}: gap residual de -6.2\%
após 23 controles (HLM~M4);
(iii)~\textit{glass ceiling de progressão}: gap cresce com o quantil salarial
(KB-test $Z=-5{,}25$, $p<0{,}001$);
(iv)~\textit{barreira contextual}: 52.5\% do gap bruto mediado pela UPA;
(v)~\textit{exclusão de redes}: betweenness nulo para todos os grupos negros
na rede expandida de 20~nós.
