...
O LRT boundary test \cite{stram1994} rejeita $H_0$ com $LR=7611,6$, $p<0{,}001$: o gap racial varia significativamente entre estados. O efeito fixo nacional é $\hat{\beta}_1=−0,1218$ (gap de 11,5\%), com $\tau^2_1=0,002561$ ($DP=0,0506$ log-pontos). Em 95\% das UFs o gap situa-se em $[−19,8\%;\,−2,2\%]$ --- variação de 17,6 pontos percentuais entre extremos.

A correlação $\rho(u_0,u_1)=−0,372$ é negativa: UFs com maior nível salarial médio (intercepto $u_0$ mais alto) exibem penalidade racial mais severa (inclinação $u_1$ mais negativa). A discriminação se intensifica nas economias mais ricas e desiguais --- e não nas mais pobres: as maiores penalidades estimadas por UF concentram-se no Distrito Federal, Rio de Janeiro e São Paulo, ao passo que estados do Nordeste (RN, PB, SE) registram as menores ($r_{\text{UF}}=+0{,}45$ entre renda média e magnitude do gap).

\medskip
\noindent\textbf{Nível adicional --- gênero.} Estendendo o \textit{random slope} a \texttt{sexo\_fem} ($(1+\text{negro}+\text{sexo\_fem}\mid \text{UF})$), o LRT confirma que o gap salarial de gênero também varia geograficamente ($LR=15.775$, $p<0{,}001$) --- com \emph{maior} dispersão que o racial ($DP_{\text{gênero}}=0,064$ vs $DP_{\text{raça}}=0,050$ log-pontos). A correlação entre as duas inclinações é negativa ($\rho=−0,46$): nas UFs de maior penalidade racial, a de gênero tende a ser menor (discriminações geograficamente compensatórias).

\subsection{Random Slope no GLMM: Heterogeneidade Geográfica do Acesso}
\label{subsec:glmm_rs}

Estendeu-se o \textit{random slope} de \texttt{negro} ao GLMM logístico de acesso (\texttt{lme4::glmer}, população completa, $N=7.694.198$), em três desfechos. O LRT de fronteira rejeita $H_0{\colon}\,\tau^2_1=0$ em todos --- \texttt{ocp\_qualif} ($LR=2.403$), \texttt{y\_top20} ($LR=322$) e \texttt{y\_top10} ($LR=502$), todos $p<0{,}001$: a barreira de acesso também varia geograficamente. A heterogeneidade é maior no acesso a ocupações qualificadas ($\tau^2_1=0,0112$, $DP=0,106$ log-odds) e preserva o gradiente de teto de vidro no efeito fixo (OR cai de 0,705 para 0,660 rumo ao decil superior).

Em contraste com o salário ($\rho=-0{,}37$), o acesso a cargos qualificados correlaciona-se \emph{positivamente} com o nível ocupacional do estado ($\rho=0,64$): UFs com mais empregos qualificados exibem \emph{menor} penalidade de acesso --- nas economias desenvolvidas, negros enfrentam acesso relativamente mais fácil, porém maior gap salarial uma vez dentro.

\textbf{Setor público {\texttimes} privado.} A barreira de acesso é praticamente idêntica (OR$_{\text{púb}}=0,705$ vs OR$_{\text{priv}}=0,695$) e igualmente heterogênea entre UFs ($\tau^2=0,0114$ vs $0,0107$): o concurso público \emph{não dissolve} a exclusão racial de acesso a ocupações qualificadas.
...
Já no \emph{salário}, o setor público \emph{atenua} o gap racial (7,8\% vs 11,2\% no privado) e o \emph{homogeneíza} entre UFs ($\tau^2_1=0,00140$ vs $0,00218$). O concurso equaliza a \emph{remuneração} de quem entra, mas não a \emph{entrada} --- precisando o diagnóstico ``o Estado ajuda, mas não resolve''.

\medskip
\noindent\textbf{Gênero no acesso.} O mesmo \textit{random slope} aplicado a \texttt{sexo\_fem} no GLMM confirma heterogeneidade geográfica em todos os desfechos ($p<0{,}001$), revelando uma divergência ausente no caso racial: mulheres têm \emph{mais} acesso a ``ocupações qualificadas'' (OR$=2,01>1$, por profissões credenciadas feminizadas), mas \emph{muito menos} acesso ao topo da renda (OR$_{\text{top20}}=0,567$; OR$_{\text{top10}}=0,522$). O teto de vidro de gênero é de \emph{remuneração}, não de categoria ocupacional --- e sua heterogeneidade geográfica supera a racial no topo ($\tau^2_{\text{sexo}}=0,0324$ no top20, contra $\tau^2_{\text{negro}}\approx0{,}003$; cerca de 10$\times$).
