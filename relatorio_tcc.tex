...
(populacao completa ($N=7.689.426$)),
testando $H_0{\colon}\,\tau^2_1 = 0$ (discriminação homogênea entre estados).

O LRT boundary test \citep{{stram1994}} rejeita $H_0$ com $LR=7611,6$, $p<0{,}001$: o gap racial varia significativamente entre estados. O efeito fixo nacional e $\hat{{\beta}}_1=−0,1218$ (gap de 11.5\%), com $\tau^2_1=0,002561$ ($DP=0,0506$ log-pontos). Em 95\% das UFs o gap situa-se em $[-19.8\%;\,-2.2\%]$ --- variacao de 17.6 pontos percentuais entre extremos.

A correlacao $\rho(u_0,u_1)=−0,372$ e negativa: estados com menor renda media apresentam maior penalidade racial, indicando que a discriminacao se concentra nas regioes mais pobres.
